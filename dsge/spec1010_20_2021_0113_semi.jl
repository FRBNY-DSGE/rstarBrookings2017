...
m <= Setting(:data_vintage, "210111")
m <= Setting(:cond_vintage, "210113") # conditional data was pulled 2 days later
m <= Setting(:cond_id, 09)
...
m <= Setting(:date_forecast_start,  quartertodate("2020-Q4"))
m <= Setting(:date_conditional_end, quartertodate("2020-Q4"))
...
if run_forecast
    ...
    df = load_data(m, cond_type = cond_type, check_empty_columns = true, try_disk = false)
